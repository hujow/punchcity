...
function Enemy.spawnOneEnemy(className, playerX, playerY)
    ...
    while not valid do
        ...
        local notInSafeZone = not Enemy.isInSafeZone(x, y, playerX, playerY, config.SAFE_ZONE_RADIUS)
    ...
    if Enemy.stepTimer >= config.ENEMY_STEP_DELAY then
        ...
        if nextX == Player.x and nextY == Player.y then
            ...
            if passable and (not occupant) then
                ...
                -- or do "slam damage" if you'd like, etc.
                Enemy.currentStepIndex = #Enemy.currentSteps  -- or #Enemy.currentSteps + 1
...
function Enemy.calculateTurnScore()
    if Enemy.enemiesHit > 0 then
        return (Enemy.enemiesHit * 10) * Enemy.totalHealthLost
    end
    return 0
end
